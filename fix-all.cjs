...
// 1. Remove Himpunan Pasangan Terurut Output
const startMarker = '{/* Himpunan Pasangan Terurut Output */}';
const startIdx = code.indexOf(startMarker);
if (startIdx !== -1) {
    const endBlock = '</div>\n              </div>\n            )}\n            \n{/* TYPE 6: BENAR DAN SALAH */}';
    let endIdx = code.indexOf(endBlock, startIdx);
    if(endIdx === -1) {
        // try alternative spacing
        const altEnd = '</div>\n              </div>\n            )}\n{/* TYPE 6: BENAR DAN SALAH */}';
        endIdx = code.indexOf(altEnd, startIdx);
    }
    if (endIdx === -1) {
        const altEnd2 = '</div>\n              </div>\n            )}\n            {/* TYPE 6: BENAR DAN SALAH */}';
        endIdx = code.indexOf(altEnd2, startIdx);
    }
    if(endIdx === -1) {
        // Just find TYPE 6
        const t6 = code.indexOf('{/* TYPE 6: BENAR DAN SALAH */}');
        const blockEnd = '</div>\n              </div>\n            )}';
        endIdx = code.lastIndexOf(blockEnd, t6);
    }
...
// 2. Add Type 7 and Type 8 if not exist
if (!code.includes('TYPE 7: ISI KOSONG')) {
    const t6Str = '{/* TYPE 6: BENAR DAN SALAH */}';
    const repStr = `            {/* TYPE 7: ISI KOSONG */}
    ...
    code = code.replace(t6Str, repStr);
}
