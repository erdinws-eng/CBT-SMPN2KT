...
export default function GuruPanel({
  ...
  const handleSaveEvaluasi = () => {
    ...
    onUpdateAttempts(allAttempts.map(a => a.id === updatedAttempt.id ? updatedAttempt : a));
    ...
    const updated = allExams.map((ex) => {
      if (ex.id === editingExam.id) {
        return {
          ...
          isActive: editExamForm.isActive !== undefined ? editExamForm.isActive : true,
        };
      }
      return ex;
    });
  ...
  const handleSaveNewExam = async (e: React.FormEvent) => {
    ...
    const createdExam: Exam = {
      ...
      token: 'DIRECT',
      status: 'active',
      isActive: true, // Exams are active by default
  ...
  // Toggle Active Exam
  const handleToggleActiveExam = (examId: string) => {
    const target = allExams.find((e) => e.id === examId);
    if (!target) return;
    const newStatus = target.isActive === false ? true : false;
    const updated = allExams.map((e) =>
      e.id === examId ? { ...e, isActive: newStatus } : e
    );
    onUpdateExams(updated);
    showSuccess(`Status ujian berhasil diubah: ${newStatus ? 'Aktif' : 'Nonaktif'}`);
  };
                  ...
                  >
                    ...
                    <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer mr-2" title={exam.isActive !== false ? 'Nonaktifkan Ujian' : 'Aktifkan Ujian'}>
                          <input
                            type="checkbox"
                            className="sr-only peer"
                            checked={exam.isActive !== false}
                            onChange={() => handleToggleActiveExam(exam.id)}
                          />
                  ...
                  <div key={att.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-center">
                        ...
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${att.status === 'violation_disqualified' ? 'bg-rose-50 text-rose-700 border border-rose-200' : att.passedKkm ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>
                          {att.status === 'violation_disqualified' ? 'Melanggar' : att.passedKkm ? 'Tuntas' : 'Tidak Tuntas'}
                        </span>
                      </div>

                      {att.status === 'violation_disqualified' && (
                        <div className="flex flex-col sm:flex-row items-center gap-1.5 border-l border-slate-200 pl-3 sm:pl-4 ml-1 sm:ml-2">
                           <button
                             onClick={() => setDeleteConfirm({ isOpen: true, type: 'reset_attempt', id: att.id, title: selectedRiwayatStudent.studentName })}
                             className="px-3 py-1.5 bg-rose-100 hover:bg-rose-200 text-rose-700 text-[11px] font-bold rounded-lg transition shadow-sm w-full sm:w-auto cursor-pointer"
                           >
                             Reset
                           </button>
                           <button
                             onClick={() => setDeleteConfirm({ isOpen: true, type: 'resume_attempt', id: att.id, title: selectedRiwayatStudent.studentName })}
                             className="px-3 py-1.5 bg-blue-100 hover:bg-blue-200 text-blue-700 text-[11px] font-bold rounded-lg transition shadow-sm w-full sm:w-auto cursor-pointer"
                           >
                             Lanjutkan
                           </button>
                        </div>
                      ...
                      {confirmingAttemptId === att.id ? (
                        <div className="flex items-center gap-1.5 bg-rose-50 border border-rose-200 p-1.5 rounded-xl">
                          ...
                          <button
                            ...
                            onClick={() => {
                              const newAttempts = allAttempts.filter(a => a.id !== att.id);
                              onUpdateAttempts(newAttempts);
                              setConfirmingAttemptId(null);
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveNewExam} className="space-y-4 text-xs">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl">
                <div>
                  <h4 className="font-bold text-slate-800 text-sm">Status Ujian</h4>
                  <p className="text-xs text-slate-500">
                    Aktifkan atau nonaktifkan ujian ini untuk siswa.
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="sr-only peer"
                    checked={newExamForm.isActive !== false}
                    onChange={(e) =>
                      setNewExamForm({
                        ...newExamForm,
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="sr-only peer"
                    checked={editExamForm.isActive !== false}
                    onChange={(e) =>
                      setEditExamForm({
                        ...editExamForm,
                        isActive: e.target.checked,
